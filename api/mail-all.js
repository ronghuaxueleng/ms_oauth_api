...
async function get_access_token(refresh_token, client_id, client_secret) {
...
module.exports = async (req, res) => {
    ...
    const { refresh_token, client_id, client_secret, email, mailbox } = req.method === 'GET' ? req.query : req.body;

    // 检查是否缺少必要的参数
    if (!refresh_token || !client_id || !email || !mailbox || !client_secret) {
        return res.status(400).json({ error: 'Missing required parameters: refresh_token, client_id, email, or mailbox' });
    }

    try {
        const access_token = await get_access_token(refresh_token, client_id, client_secret);
